...
#define MORUS_1280_TAG_LEN_WORDS	2
		...
		static void _updateState(array<array<UInt64>^>^ state, array<const UInt64>^ msgblk)
		{
			UInt64 temp;
			...
			state[0][0] = RotL_64(state[0][0], ROTL64C1);  state[0][1] = RotL_64(state[0][1], ROTL64C1);       state[0][2] = RotL_64(state[0][2], ROTL64C1);       state[0][3] = RotL_64(state[0][3], ROTL64C1);
			...
			state[1][0] = RotL_64(state[1][0], ROTL64C2);  state[1][1] = RotL_64(state[1][1], ROTL64C2);       state[1][2] = RotL_64(state[1][2], ROTL64C2);       state[1][3] = RotL_64(state[1][3], ROTL64C2);
			...
			state[2][0] = RotL_64(state[2][0], ROTL64C3);  state[2][1] = RotL_64(state[2][1], ROTL64C3);       state[2][2] = RotL_64(state[2][2], ROTL64C3);       state[2][3] = RotL_64(state[2][3], ROTL64C3);
			...
			state[3][0] = RotL_64(state[3][0], ROTL64C4);  state[3][1] = RotL_64(state[3][1], ROTL64C4);       state[3][2] = RotL_64(state[3][2], ROTL64C4);       state[3][3] = RotL_64(state[3][3], ROTL64C4);
			...
			state[4][0] = RotL_64(state[4][0], ROTL64C5);  state[4][1] = RotL_64(state[4][1], ROTL64C5);       state[4][2] = RotL_64(state[4][2], ROTL64C5);       state[4][3] = RotL_64(state[4][3], ROTL64C5);
			temp = state[2][3];    state[2][3] = state[2][2];  state[2][2] = state[2][1];  state[2][1] = state[2][0];  state[2][0] = temp;
		}

		static array<array<UInt64>^>^ _init(array<const Byte>^ key, array<const Byte>^ iv)
		{
			array<array<UInt64>^>^ state = gcnew array<array<UInt64>^>(5); // 5 rows
			state[0] = gcnew array<UInt64>(4); // iv goes in front half here
			state[1] = gcnew array<UInt64>(4); // key goes here (twice if 16 bytes (128bits), once if 32 bytes (256 bits))
			state[2] = gcnew array<UInt64>{ UInt64::MaxValue, UInt64::MaxValue, UInt64::MaxValue, UInt64::MaxValue };
			state[3] = gcnew array<UInt64>(4); // all 0's
			state[4] = gcnew array<UInt64>{CONST64_0_0, CONST64_0_1, CONST64_1_0, CONST64_1_1}; // constant goes here
			...
			if (key->Length == 16)
			{
				Buffer::BlockCopy(key, 0, state[1], 0, key->Length); // key fills first 16 bytes...
				Buffer::BlockCopy(key, 0, state[1], 16, key->Length); // ...repeated in second 16 bytes
			}
			else if(key->Length == 32)
				Buffer::BlockCopy(key, 0, state[1], 0, key->Length); // key fills all 32 bytes (different from 1280-128)

			array<UInt64>^ tempKey = gcnew array<UInt64>{ state[1][0], state[1][1], state[1][2], state[1][3] }; // need temp key down below
			Byte i;
			for (i = 0; i < 16; i++) _updateState(state, (array<const UInt64>^)temp);
		...
		static __inline void _finalize_state(array<array<UInt64>^>^ state, const UInt64 adlen, const UInt64 msglen)
		{
			...
			array<const UInt64>^ t = { adlen << 3, msglen << 3, 0, 0 };
			for (Byte i = 0; i < 10; i++) _updateState(state, t);
			...
			MemoryStream^ Plaintext) 
		{
			...
			for (Byte i = 0; i < 5; i++)
			{
				...
				state[4][0] |= (state[i][0] | UInt64::MaxValue) + compare;
			}
